...
- **LEGACY FLOW NOTE (final):** app config confirmed `use legacy install flow: true` → the Admin API token is **only minted when the app is installed on the store**. Every token type was tested and rejected (client secret `shpss_…`, `b22f9e4e…`, webhook signing secret `f482fba2…`, app automation tokens `atkn_f18b6d…` + `atkn_f82ec48b…`): all 401/400. No API token can exist until the app is installed — registration of the 3 app-level topics is blocked on that single browser action; the script `register-shopify-app-topics.sh` is ready to fire the moment a valid `shpat_…` appears.
