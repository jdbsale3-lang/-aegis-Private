...
- **VERIFICATION (30 Aug 2026, receiver-side):** 37 deliveries logged · all 10 topics present with ≥1 real/signed-verified delivery each — `orders/paid` 3 real events (incl. genuine Shopify-sent order 1234, £404.95), the other 9 topics 1 real each (Shopify test notifications). Proof of live end-to-end from the store.
- **LEGACY FLOW NOTE (final):** app config confirmed `use legacy install flow: true` → the Admin API token is **only minted when the app is installed on the store**. Every token type was tested and rejected — client secrets (`shpss_40bec…`, `shpss_031591…`, `b22f9e4e…`), webhook signing secret (`f482fba2…`), automation tokens (`atkn_f18b6d…`, `atkn_f82ec48b…`, `atkn_08b309…`), hash key id (`641999b5…`) across stores `xegrdn-7v` + `zeusai2026`, api 2024-10 + 2026-07 — all 401/400. No API token can exist until the app is **installed in the store admin** (browser action). **3 app-level topics (`app/uninstalled`, `app/scopes_update`, `checkouts/paid`) = DEFERRED/OPTIONAL — no functional impact on income flow; script `register-shopify-app-topics.sh` is ready if a valid `shpat_…` ever appears.**
- **Shopify 2026-07 webhook signature scheme (HTTP Message Signatures):** Shopify may attach `Signature` / `Signature-Input` headers (label e.g. `maifri`, key id per store) in addition to `X-Shopify-Hmac-Sha256`. The receiver ignores these extra headers — HMAC verification passes and deliveries are 200-acked (verified with real deliveries carrying them). No change required; noted for future monitoring.
- **FINAL STATUS: Shopify integration COMPLETE for all functional purposes** (10/10 store webhooks live, receipts → Sheets, Accounts booking).
