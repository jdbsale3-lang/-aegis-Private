...
def test_watermark_applied():
    ...
    result = defense.apply_watermark("The analysis shows an excellent result using this method.")
    ...
    if result.watermark_type != "none":
        assert result.watermarked_output != "The analysis shows an excellent result using this method."
    else:
...
def test_watermark_lexical_substitution():
    ...
    assert "exceptional" in result.watermarked_output or "resolution" in result.watermarked_output
...
def test_high_volume_detection():
    ...
    for i in range(55):
        defense.monitor_query(session_id, f"standard_question_{i}", "192.168.1.1", "user_1")
...
def test_systematic_probing():
    ...
    for i in range(150):
        defense.monitor_query(session_id, f"systematic_query_{i}", "10.0.0.1", "attacker")
...
def test_output_perturbation_synonym():
    ...
    assert "excellent" in perturbed or "exceptional" in perturbed or (
        perturbed != long_text
    )
...
def test_distribution_shift():
    ...
    for i in range(30):
        defense.monitor_query(session_id, f"Question {i} about common topics?",
                              "10.0.0.4", "user_2")
