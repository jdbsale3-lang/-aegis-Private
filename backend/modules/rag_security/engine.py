...
class RAGSecurityEngine:
    ...
    ADVERSARIAL_PATTERNS = [
        # Hidden instructions
        (
            r"ignore\s+(all\s+)?(previous|prior|above)\s+(instructions|context|documents)",
            "hidden_instruction",
        ),
        (
            r"(disregard|forget|override)\s+(your\s+)?(instructions|context|knowledge)",
        ...
        (
            r"(however|but|contrary|nevertheless|despite)\s*.*\b(actually|really|truly)\b",
            "contradictory_signal",
        ),
        # Injection payload indicators
        (r"<script>|</script>|<iframe>|javascript\s*:", "xss_in_document"),
        (
            r"SELECT\s+.*\s+FROM\s+.*\s+WHERE|DROP\s+TABLE|UNION\s+SELECT",
            "sql_injection",
        ),
        # Poisoning markers
                ...
                threats.append(
                    {
                        ...
                        "severity": (
                            "critical"
                            if "injection" in threat_type or "override" in threat_type
                            else "high"
                        ),
                    }
                )
    ...
    def _find_repeated_suspicious_phrases(
        self, text: str, min_phrase_len: int = 5
    ) -> list[str]:
    ...
    def scan_document(self, document_id: str, text: str) -> IngestionReport:
        ...
        for chunk in chunks:
            ...
            for t in threats:
                ...
                if "severity" in t:
                    risk_score = max(
                        risk_score, severity_weights.get(t["severity"], 0.5)
                    )
    ...
    def _detect_temporal_anomaly(self, chunks: list[dict]) -> float:
        ...
        recent_count = sum(
            1
            for c in chunks
            if c.get("ingested_at", 0) > 0
            and c.get("ingested_at", 0) < 3600  # last hour
        )
    ...
    def trace_query(
        self, query_id: str, query: str, retrieved_chunks: list[dict]
    ) -> QueryTraceResult:
        """
        ...
        for chunk in retrieved_chunks:
            threats = self._scan_chunk_for_threats(
                DocumentChunk(
                    text=chunk.get("text", ""),
                    index=0,
                    hash=hashlib.sha256(chunk.get("text", "").encode()).hexdigest()[
                        :16
                    ],
                )
            )
        ...
        if relevance_anomaly > self.anomaly_threshold:
            parts.append(
                f"Relevance anomaly: low-confidence document dominates response. Score: {relevance_anomaly:.2f}"
            )
        if temporal_anomaly > self.anomaly_threshold:
            parts.append(
                f"Temporal anomaly: recently ingested documents dominate response. Score: {temporal_anomaly:.2f}"
            )
        if content_anomaly > self.anomaly_threshold:
            parts.append(
                f"Content anomaly: adversarial patterns detected in retrieved chunks ({len(poisoned_chunks)} chunks)"
            )
